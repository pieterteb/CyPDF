...
static void CYPDF_PageNodeAddKid(CYPDF_ObjPageNode* const restrict page_node, CYPDF_ObjPage* const restrict page);
...
static void CYPDF_PageNodeAddKid(CYPDF_ObjPageNode* const restrict page_node, CYPDF_ObjPage* const restrict page) {
    ...
    if (page_node && page) {
        CYPDF_ObjArray* kids = CYPDF_DictGetValueIndex(page_node, CYPDF_PAGE_NODE_KIDS_I);
        CYPDF_ArrayAppend(kids, page);
        CYPDF_ObjInteger* leaf_count = CYPDF_DictGetValueIndex(page_node, CYPDF_PAGE_NODE_COUNT_I);
...
void CYPDF_PageAddContent(CYPDF_ObjPage* const restrict page, CYPDF_ObjStream* const restrict stream) {
    ...
    if (stream && page) {
        CYPDF_ObjArray* contents = CYPDF_DictGetValueIndex(page, CYPDF_PAGE_CONTENTS_I);
