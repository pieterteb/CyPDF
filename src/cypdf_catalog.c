...
CYPDF_Obj_Catalog* CYPDF_New_Catalog(CYPDF_BOOL indirect, CYPDF_Obj_PNode* pages) {
    CYPDF_Obj_Catalog* catalog = (CYPDF_Obj_Catalog*)CYPDF_New_Obj(indirect, CYPDF_OCLASS_CATALOG);
    if (catalog) {
        catalog->dict = CYPDF_New_Dict(CYPDF_TRUE);
        if (catalog->dict) {
            /* The type of PDF object that this dictionary describes. */
            CYPDF_Obj_Name* type = CYPDF_New_Name(CYPDF_FALSE, "Catalog");
